...
RSpec.describe TikaFileCharacterizationService do
  ...
  let(:original_file) { query_service.find_by(id: valid_file_set.original_file.proxy.first) }
  ...
  it 'characterizes a sample file' do
    described_class.new(file_node: original_file, persister: persister).characterize
  end

  it 'sets the height attribute for a file_node on characterize ' do
    t_file_node = original_file
  ...
  it 'does not save to the persister when characterize is called with save false' do
    allow(persister).to receive(:save).and_return(original_file)
    described_class.new(file_node: original_file, persister: persister).characterize(save: false)
